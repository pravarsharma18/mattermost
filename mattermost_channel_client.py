...
class MattermostClient:
    ...
    def insert_chats(self):
            channel_keys = ['id', 'createat', 'updateat', 'deleteat', 'teamid', 'type', 'displayname', 'name',
                            'header', 'purpose', 'lastpostat', 'totalmsgcount', 'extraupdateat', 'creatorid', 'shared', 'totalmsgcountroot', 'lastrootpostat']
                    ...
                    if len(rec_ids) == 2 and not chat['chat_id'].startswith('CT'):
                        ...
                        if creator_id:
                            channel_values = [
                                ...
                                "D", "", rec_ids_str, "", chat['chat_id'], int(float(chat['last_modified_time'])), 0, 0, creator_id[0]['id'], 
                                json.dumps(False), 0, int(float(chat['last_modified_time']))]
                            channel_values_reverse = [
                                self.generate_id(26), int(float(chat['creation_time'])), int(float(chat['last_modified_time'])), 0, "",
                                "D", "", rec_ids_str_reverse, "", chat['chat_id'], int(float(chat['last_modified_time'])), 0, 0, creator_id[0]['id'], 
                                json.dumps(False), 0, int(float(chat['last_modified_time']))]
    ...
    def insert_posts(self):
            ...
            for mm_channel in mm_channels:
                zoho_cliq_messages = ZohoSqlClient.sql_get('cliq_messages', params=f"chat_id='{mm_channel.get('purpose')}' and is_processed = false")
                for zoho_cliq_message in zoho_cliq_messages:
                    try:
                        ...
                        ZohoSqlClient.sql_update(table_name="cliq_messages", set="is_processed = true", where=f"chat_id = '{mm_channel.get('purpose')}'")
                    except Exception as e:
